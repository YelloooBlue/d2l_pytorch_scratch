...
def get_params(vocab_size, num_hiddens, device):
    ...
    # 重置门参数
    W_xr = torch.randn((num_inputs, num_hiddens), device=device) * 0.01  # 输入到重置门的权重
    W_hr = torch.randn((num_hiddens, num_hiddens), device=device) * 0.01  # 隐藏层到重置门的权重
    b_r = torch.zeros(num_hiddens, device=device)  # 重置门偏置

    # 更新门参数
    W_xz = torch.randn((num_inputs, num_hiddens), device=device) * 0.01  # 输入到更新门的权重
    W_hz = torch.randn((num_hiddens, num_hiddens), device=device) * 0.01  # 隐藏层到更新门的权重
    b_z = torch.zeros(num_hiddens, device=device)  # 更新门偏置
...
def gru(inputs, state, params):
    """实现GRU的前向计算"""
    W_xz, W_hz, b_z = params[0:3]  # 更新门参数
    W_xr, W_hr, b_r = params[3:6]  # 重置门参数
    W_xh, W_hh, b_h = params[6:9]  # 隐藏层参数
    W_hq, b_q = params[9:11]  # 输出层参数

    outputs = []

    for X in inputs:
        # 计算更新门
        z = torch.sigmoid(X @ W_xz + state @ W_hz + b_z)
        # 计算重置门
        r = torch.sigmoid(X @ W_xr + state @ W_hr + b_r)
        # 计算候选隐藏状态
        h_tilde = torch.tanh(X @ W_xh + (r * state) @ W_hh + b_h)
        # 更新隐藏状态
...
if __name__ == '__main__':
    ...
    # =========== 实例化模型 ===========
    net = GRUModel(vocab_size, num_hiddens)
    ...
    """
        加入门控GRU，只需要改变上一节中我们定义模型的 forward 函数即可。
    """