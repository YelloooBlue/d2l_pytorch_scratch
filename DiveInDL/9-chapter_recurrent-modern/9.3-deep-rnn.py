...
def get_params(vocab_size, num_hiddens, device):
    ...
    # 隐藏层1参数
    W_xh1 = torch.randn((num_inputs, num_hiddens), device=device) * 0.01
    W_h1h1 = torch.randn((num_hiddens, num_hiddens), device=device) * 0.01
    b_h1 = torch.zeros(num_hiddens, device=device)

    # 隐藏层2参数
    W_h1h2 = torch.randn((num_hiddens, num_hiddens), device=device) * 0.01 # 注意这里的输入维度
    W_h2h2 = torch.randn((num_hiddens, num_hiddens), device=device) * 0.01
...
def deeprnn(inputs, state, params):
    """实现深度RNN的前向计算"""
    W_xh1, W_h1h1, b_h1 = params[0:3]
    W_h1h2, W_h2h2, b_h2 = params[3:6]
    W_hq, b_q = params[6:8]
    ...
    for X in inputs:
        ...
        h1 = torch.tanh(X @ W_xh1 + h1 @ W_h1h1 + b_h1) # 接受当前输入X，和上一时间步的h1
        h2 = torch.tanh(h1 @ W_h1h2 + h2 @ W_h2h2 + b_h2) # 接受上一层h1，和上一时间步的h2
...
class RNNModelScratch:
    ...
    def __call__(self, X, state):
        ...
        return deeprnn(X, state, self.params)
...
if __name__ == '__main__':
    ...
    h1 = torch.zeros((test_batch_size, num_hiddens), device=device)  # 隐藏层1的状态
    h2 = torch.zeros((test_batch_size, num_hiddens), device=device)  # 隐藏层2的状态
    state = (h1, h2)  # 状态是一个元组，包含两个隐藏层的状态
    ...
    print(f'状态长度: {len(state_new)}')  # 状态是一个元组，长度为2
    print(f'新状态1形状: {state_new[0].shape}')  # 新状态的形状是 (batch_size, num_hiddens)
    print(f'新状态2形状: {state_new[1].shape}')  # 新状态的形状是 (batch_size, num_hiddens)
    print(" ===== 模拟前向传播结束 ===== ")
    ...
    with torch.no_grad(): # 虽然不会影响梯度计算，但可以减少内存使用
        ...
        h1 = torch.zeros((1, num_hiddens), device=device)  # 隐藏层1的状态
        h2 = torch.zeros((1, num_hiddens), device=device)  # 隐藏层2的状态
        state = (h1, h2)  # 状态是一个元组，包含两个隐藏层的状态
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in train_iter:
            ...
            if state is None:
                # 初始化状态
                h1 = torch.zeros((X.shape[0], num_hiddens), device=device)
                h2 = torch.zeros((X.shape[0], num_hiddens), device=device)
                state = (h1, h2)
            else:
                # state = state.detach()  # detach状态，避免梯度累积
                for s in state:
                    s.detach_() # 注意s = s.detach()是不可行的，因为tuple是不可变的
    ...
    with torch.no_grad():
        ...
        h1 = torch.zeros((1, num_hiddens), device=device)
        h2 = torch.zeros((1, num_hiddens), device=device)
        state = (h1, h2)
