...
class TextCNN(nn.Module):
    def __init__(self, vocab_size, embed_size, kernel_sizes, num_channels, **kwargs):
        ...
        for c, k in zip(num_channels, kernel_sizes):
            self.convs.append(nn.Conv1d(2 * embed_size, c, k)) # 这个卷积层的输入通道数是2 * embed_size，输出通道数是c，卷积核的大小是k

        """
            不同kernel_sizes宽度的卷积核可以捕获不同数目的相邻词元之间的局部特征
        """
    ...
    def forward(self, inputs):
        ...
        """
            embeddings的形状是（批量大小，词元向量维度，时间步数=词元数量），
            conv(embeddings)的输出形状是（批量大小，通道数，时间步数），「从嵌入空间到多通道空间」
            self.pool(conv(embeddings))的输出形状是（批量大小，通道数，1），
            torch.squeeze(self.pool(conv(embeddings)), dim=-1)的输出形状是（批量大小，通道数），
            torch.cat([...], dim=1)的输出形状是（批量大小，通道数总和），
        """
